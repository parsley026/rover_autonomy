...
from tutorial_interfaces.msg import ArucoPosition
import cv2
from py_srvcli.CameraHandler import CameraHandler 

class ArucoPositionNode(Node):
    def __init__(self):
        super().__init__('aruco_position_')
        ...
        self.subscription = self.create_subscription(
            Image,
            '/hikvision_camera_node/image_raw',  # Zmień, jeśli CameraNode działa z inną przestrzenią nazw
            self.image_callback,
        ...
        self.pose_publisher = self.create_publisher(
            ArucoPosition, 
            '/aruco/estimated_pose', 
            10
        )
        
        self.get_logger().info("Gotowy do pracy. Publikuję pozycję!")

        matrix_path = '/home/dudziu/Desktop/Raptors/arucoERC26/src/py_srvcli/py_srvcli/metadata/ZZ_camera_matrix.npy'
        dist_path = '/home/dudziu/Desktop/Raptors/arucoERC26/src/py_srvcli/py_srvcli/metadata/ZZ_dist_coeffs.npy'

        try:
            self.camera_matrix = np.load(matrix_path)
            self.dist_coeffs = np.load(dist_path)
            self.get_logger().info("Pomyślnie załadowano macierz kamery i dystorsję!")
            
            # (Opcjonalnie) Obliczamy optymalną macierz, żeby nie ucięło krawędzi.
            # Podaj rozdzielczość, w jakiej nadaje Twoja kamera (np. 1920x1080 lub 640x480)
            szerokosc = 1920 
            wysokosc = 1080
            self.new_camera_matrix, _ = cv2.getOptimalNewCameraMatrix(
                self.camera_matrix, self.dist_coeffs, (szerokosc, wysokosc), alpha=0
            )
            self.is_calibrated = True

        except Exception as e:
            self.get_logger().error(f"Nie udało się wczytać plików .npy: {e}")
    ...
    def image_callback(self, msg: Image):
        ...
        request = ArucoEstimate.Request()
        response = ArucoEstimate.Response()
        ...
        try:
            ...
            if self.is_calibrated:
                self.latest_frame = cv2.undistort(
                    raw_frame, 
                    self.camera_matrix, 
                    self.dist_coeffs, 
                    None, 
                ...
                self.latest_frame = raw_frame # Fallback, jeśli brak plików
            ...
            if self.prev_response is not None:
                request.prev_x = self.prev_response.x
                request.prev_y = self.prev_response.y
                request.prev_z = self.prev_response.z

            self.estimate_callback(request, response)

            if response.total_success:
                print(f"\n[INFO] Wyniki analizy z klatki:\n  Pozycja X: {response.x:.3f} m\n  Pozycja Y: {response.y:.3f} m\n  Wysokość Z: {response.z:.3f} m\n  Kąt łazika (Yaw): {response.z_angle:.2f}°\n Kąt łazika (Pitch): {response.y_angle:.2f}°\n  Kąt łazika (Roll): {response.x_angle:.2f}°\n Liczba wykrytych ArUco w bazie: {response.arucos}\n  Sukces analizy: {'TAK' if response.total_success else 'NIE'}\n")
                self.prev_response = response

                msg_out = ArucoPosition()
                msg_out.x = response.x
                msg_out.y = response.y
                msg_out.z = response.z
                msg_out.x_angle = response.x_angle
                msg_out.y_angle = response.y_angle
                msg_out.z_angle = response.z_angle
                msg_out.arucos = response.arucos
                msg_out.total_success = response.total_success

                self.pose_publisher.publish(msg_out)


                if response.arucos >= 2:
                    self.image_limiter += 1

                    if(self.image_limiter >= 10):
                        self.image_limiter = 0
    ...
    def estimate_callback(self, request, response):
        ...
        # Zapisanie wyników do odpowiedzi node'a
