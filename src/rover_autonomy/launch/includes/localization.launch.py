...
def launch_setup(context, *args, **kwargs):
    ...
    localization_mode = str(LaunchConfiguration('localization_mode').perform(context))
    ...
    if localization_mode == '111':
        # 111: Full Fusion (LiDAR Pose + Cam0 Velocity + Cam1 Velocity)
        mode_params = {
    ...
    elif localization_mode == '110':
        # 110: LiDAR + Cam0 (LiDAR Pose + Cam0 Velocity)
        mode_params = {
            'odom0': f"/{lidar_ns}/odom",  'odom0_config': odom_pose_config,  'odom0_relative': True,
            'odom1': f"/{cam0_ns}/odom",   'odom1_config': odom_twist_config, 'odom1_relative': False,
            'imu0': f"/{lidar_ns}/imu",    'imu0_config': imu_config, 'imu0_queue_size': 50, 'imu0_relative': False, 'imu0_remove_gravitational_acceleration': True,
        }
    elif localization_mode == '101':
        # 101: LiDAR + Cam1 (LiDAR Pose + Cam1 Velocity)
        mode_params = {
            'odom0': f"/{lidar_ns}/odom",  'odom0_config': odom_pose_config,  'odom0_relative': True,
            'odom1': f"/{cam1_ns}/odom",   'odom1_config': odom_twist_config, 'odom1_relative': False,
            'imu0': f"/{lidar_ns}/imu",    'imu0_config': imu_config, 'imu0_queue_size': 50, 'imu0_relative': False, 'imu0_remove_gravitational_acceleration': True,
        }
    elif localization_mode == '100':
        # 100: LiDAR Only (LiDAR Pose)
        mode_params = {
            'odom0': f"/{lidar_ns}/odom",  'odom0_config': odom_pose_config,  'odom0_relative': True,
            'imu0': f"/{lidar_ns}/imu",    'imu0_config': imu_config, 'imu0_queue_size': 50, 'imu0_relative': False, 'imu0_remove_gravitational_acceleration': True,
        }
    elif localization_mode in ['011', '11', '9']:
        # 011: Dual Camera (Cam0 Pose + Cam1 Velocity)
        mode_params = {
            'odom0': f"/{cam0_ns}/odom",   'odom0_config': odom_pose_config,  'odom0_relative': True,
            'odom1': f"/{cam1_ns}/odom",   'odom1_config': odom_twist_config, 'odom1_relative': False,
        }
    elif localization_mode in ['010', '10', '8']:
        # 010: Cam0 Only (Cam0 Pose)
        mode_params = {
            'odom0': f"/{cam0_ns}/odom",   'odom0_config': odom_pose_config,  'odom0_relative': True,
        }
    elif localization_mode in ['001', '1']:
        # 001: Cam1 Only (Cam1 Pose)
        mode_params = {
            'odom0': f"/{cam1_ns}/odom",   'odom0_config': odom_pose_config,  'odom0_relative': True,
        }
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('localization_mode', default_value='110',               description='Localization Mode (Binary: LiDAR-Cam0-Cam1, e.g. 110)'),
