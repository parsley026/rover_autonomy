...
def launch_setup(context, *args, **kwargs):
    ...
    launch_aruco   = get_flag('launch_aruco', 'false')
    aruco_config   = get_config_path('aruco_config',   'config/localization/aruco_localization.yaml')
    aruco_landmarks = get_config_path('aruco_landmarks', 'config/localization/landmarks.yaml')
    ...
    aruco_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(pkg_share, 'launch', 'includes', 'aruco_localization.launch.py')),
        launch_arguments={
            'camera_ns':      'camera_00',
            'params_file':    aruco_config,
            'landmarks_file': aruco_landmarks,
            'use_sim_time':   use_sim_time,
        }.items(),
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        # Camera decode overrides (sourced from bringup_profile.yaml by default)
        ...
        # ArUco localization overrides (sourced from bringup_profile.yaml by default)
        DeclareLaunchArgument('launch_aruco',     default_value='default'),
        DeclareLaunchArgument('aruco_config',     default_value='default'),
        DeclareLaunchArgument('aruco_landmarks',  default_value='default'),
